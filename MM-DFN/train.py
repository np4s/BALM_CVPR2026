...
def train(model: nn.Module,
    ...
    for epoch in range(args.epochs):
        ...
        for idx in (pbar := tqdm(range(len(train_set)), desc=f"Epoch {epoch}, CLS {current_loss:,.4f}, Mod. {mod_loss:,.4f}")):
            ...
            if len(modals) > 1 and args.mm_fusion_mthd in ['gated', 'mfn', 'mfn_only', 'concat_subsequently', 'tfn_only', 'lmf_only', 'concat_only']:
                log_prob, smax_feat, enc = model(textf, qmask, length, acouf, visuf,
                                                 amask=amask, vmask=vmask, lmask=lmask)
            else:
                log_prob = model(textf, qmask, length)
...
def evaluate(model, dataset, loss_fn, args, logger=None, test=True):
    ...
    with torch.no_grad():
        ...
        for idx in range(len(dataset)):
            ...
            # transpose (batch, seq_len, feature_dim) -> (seq_len, batch, feature_dim)
            length = data["length"].tolist()
            acouf = data['tensor']['a'].to(device).transpose(0, 1)
            textf = data['tensor']['t'].to(device).transpose(0, 1)
            visuf = data['tensor']['v'].to(device).transpose(0, 1)
        ...
        if 'mosei'==args.dataset:
            non_zeros = np.array([i for i, e in enumerate(golds) if e != 0]) # remove 0
...
def get_argurment():
    ...
    parser.add_argument("--d_global", type=int, default=737,
                        help="Dimension of global feature in FCM module, set to (dim_a + dim_v + dim_l)/(num_modals+1) for simplicity")
    ...
    parser.add_argument("--modal_MR", default=None, type=float,
                        nargs=3, help="Modal missing rate for imperfect data training")
    ...
    parser.add_argument('--base_model', default='LSTM',
                        help='base recurrent model, must be one of DialogRNN/LSTM/GRU/None')
    parser.add_argument('--graph_model', action='store_true', default=True,
                        help='whether to use graph model after recurrent encoding')
    parser.add_argument('--graph_type', default='GDF',
                        help='relation/GCN3/DeepGCN/GF/GF2/GDF')
    ...
    parser.add_argument('--nodal_attention', action='store_true',
                        default=True, help='whether to use nodal attention in graph model')
    parser.add_argument('--use_topic', action='store_true',
                        default=False, help='whether to use topic information')
    ...
    parser.add_argument('--active_listener', action='store_true',
                        default=False, help='active listener')
    parser.add_argument('--attention', default='general',
                        help='Attention type in DialogRNN model')
    parser.add_argument('--use_crn_speaker', action='store_true',
                        default=True, help='whether to use use crn_speaker embedding')
    parser.add_argument('--speaker_weights', type=str,
                        default='3-0-1', help='speaker weight 0-0-0')
    parser.add_argument('--use_speaker', action='store_true',
                        default=False, help='whether to use speaker embedding')
    parser.add_argument('--reason_flag', action='store_true',
                        default=False, help='reason flag')
    parser.add_argument('--modal_weight', type=float,
                        default=1.0, help='modal weight 1/0.7')
    parser.add_argument('--Deep_GCN_nlayers', type=int,
                        default=16, help='Deep_GCN_nlayers')
...
def main(args):
    ...
    args.model_name = f'MMDFN_{args.dataset}_{args.seed}_{args.batch_size}_{args.epochs}_{mr}'
    ...
    if 'mosei'==args.dataset:
        ...
        loss_fn = FocalLoss(gamma=args.focal, alpha=class_weights)
    else:
        loss_fn = nn.NLLLoss(class_weights)
